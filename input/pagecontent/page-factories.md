...
on the item being iterated, and then replaces all occurrences of 
the text string given in the name with the transformed value.
...
* **tla**: For resource names, look up the known TLA (see the [resource-tla](StructureDefinition-resource-tla.html) extension) for the version
